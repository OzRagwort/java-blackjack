...
public class ResultView {
    ...
    public static void printProfitResults(ProfitResults results) {
        System.out.println(PRINT_PROFIT_RESULTS_MESSAGE);

        int dealerProfit = results.getDealerProfit();
        printPlayerProfitState(DEALER_NAME, dealerProfit);

        for (Player player : results.getPlayers()) {
            Profit profit = results.get(player);
            printPlayerProfitState(player.getName().get(), profit.get());
        }
    }

    private static void printPlayerProfitState(String name, int profit) {
        System.out.println(name + PRINT_RESULTS_DELIMITER + profit);
    }
